                ...
                return (
                  <div key={close.id} className={`bg-white rounded-2xl border transition-all duration-300 overflow-hidden group ${isExpanded ? 'border-blue-200 shadow-md ring-1 ring-blue-100' : 'border-slate-100 shadow-sm hover:shadow-md hover:border-blue-100'}`}>
                    ...
                    >
                      ...
                      {/* Info Grid */}
                      <div className="flex-1 grid grid-cols-2 md:grid-cols-4 gap-y-4 gap-x-8 items-center">
                       ...
                       <div className="border-t border-slate-100 bg-slate-50/50 p-4 md:p-6 animate-in slide-in-from-top-2 duration-300">
                          ...
                          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            {month.closings.map(close => (
                              <div key={close.id} className="bg-white p-4 rounded-xl border border-slate-200 shadow-sm flex items-center justify-between">
                                 <div className="flex items-center gap-3">
                                    <div className="w-10 h-10 rounded-full bg-slate-100 flex items-center justify-center font-bold text-slate-600 text-sm">
                                      {new Date(close.date).getDate()}
                                    </div>
                                    <div>
                                      <p className="font-bold text-sm text-slate-800">{new Date(close.date).toLocaleDateString('es-MX', { weekday: 'long' })}</p>
